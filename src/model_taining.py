...
def split_data(data, by, on):
    ...
    if by == "country":
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            data[feature_columns], data["R"], test_size=0.2
        )
...
def get_output_name(folder, name, suffix, extension):
    return (
        "{}/{}.{}".format(folder, name, extension)
        if suffix == ""
        else "{}/{}_{}.{}".format(folder, name, suffix, extension)
    )
...
def train_mlp(data, split_by="random", split_on=None, output_suffix=""):
    ...
    save_model(
        model, reports, y_std, scaler, list(X_train.columns), data, output_suffix
    )
...
if __name__ == "__main__":
    ...
    data = pd.read_csv(CONFIG["common"]["paths"]["ml_features"], low_memory=False).drop(["Unnamed: 0"], axis=1)
    train_mlp(data)
